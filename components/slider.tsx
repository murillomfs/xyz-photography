...
export function Slider() {
    ...
    return(
        <div className="slider relative w-[512px] h-[680px] max-h-[80vh] z-[3]">
            { slides.map((slide: Slide, index: number) => (
                <div key={ index } className={ `slide ${ index == 0 ? 'slide-active' : 'slide-next' } absolute flex items-center justify-center w-full h-full top-0 left-0` }>
                    <div className="slide-image absolute flex items-center justify-center w-full h-full top-0 left-0 border-black border rounded-[10px] overflow-hidden">
                        ...
                        <div className="slide-title slide-content absolute text-white font-semibold text-[220px] uppercase tracking-[8px] leading-[80%] text-center" dangerouslySetInnerHTML={{ __html: slide?.project}}></div>
                    </div>

                    <div className="slide-title-stroke slide-content absolute font-semibold text-[220px] uppercase tracking-[8px] leading-[80%] text-center" dangerouslySetInnerHTML={{ __html: slide?.project}}></div>
                </div>
            )) }

            <div className="slider-control absolute flex gap-6 items-center w-full justify-center bottom-[100px] left-0">
